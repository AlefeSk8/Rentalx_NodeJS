...
let CreateRentalUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (carUnavailable) {
      throw new _AppError.AppError("Car is unavailable", 401);
    }
    ;
    const userOpenRental = await this.rentalsRepository.findOpenRentalByUser(user_id);
    if (userOpenRental) {
      throw new _AppError.AppError("Already have a rental in progress for this user!", 401);
    }
    ;
    const dateNow = this.dateProvider.dateNow();
    const compare = this.dateProvider.compareInHours(dateNow, expected_return_date);
    if (compare < minimumHours) {
      throw new _AppError.AppError("Invalid return time!", 401);
    }
    ;
